...
document.addEventListener('DOMContentLoaded', function() {
  const works = document.querySelectorAll('.single-work');

  works.forEach((work) => {
    const zoomRange = work.querySelector('.zoom-range');
    const zoomImage = work.querySelector('.zoom-image');

    if (zoomRange && zoomImage) {
      zoomRange.addEventListener('input', function() {
        const scaleValue = 1 + (zoomRange.value * 0.10); // Calculate the scale value
        zoomImage.style.transform = `scale(${scaleValue})`;
      });

      zoomRange.addEventListener('change', function() {
        setTimeout(() => {
          zoomRange.value = 0; // Reset the slider to the start
          zoomImage.style.transform = `scale(1)`; // Reset the image scale to normal
        }, 300); // Adjust the timeout duration as needed
      });
    } else {
      console.error("zoomRange or zoomImage not found");
    }
  });
});
/*--------------------------------------------------------------------------------------------*/
document.addEventListener('DOMContentLoaded', (event) => {
  const skillElement = document.querySelector('#skill-typed');
  const logos = {
    "Photoshop": "assets/images/logo/photoshop.png",
    "Canva": "assets/images/logo/canva.png",
    "HTML": "assets/images/logo/html.png",
    "CSS": "assets/images/logo/css.png",
    "JavaScript": "assets/images/logo/javascript.png",
    "After Effect": "assets/images/logo/after_effect.png",
    "Davinci Resolve": "assets/images/logo/davinci_resolve.png",
    "Capcut": "assets/images/logo/capcut.png",
    "Figma": "assets/images/logo/figma.png",
    "ChatGPT": "assets/images/logo/chatgpt.png",
    "Illustrator": "assets/images/logo/illustrator.png"
  };
  ...
  if (skillElement) {
    ...
    const stringsWithLogos = typedStrings.map(text => `<span class="underline">${text} <img src="${logos[text]}" alt="${text} Logo" class="logo"></span>`);

    new Typed('#skill-typed', {
      strings: stringsWithLogos,
      typeSpeed: 50,
      backSpeed: 50,
      loop: true,
      onStringTyped: (arrayPos, self) => {
        // Add delay before starting to backspace
        self.stop();
